# IAM for Phase 6: EventBridge needs permission to start Step Functions
...
# ─── Data source: look up the Phase 5 state machine ───────────────────
# We reference the state machine by name rather than hardcoding the ARN.
# This keeps the phases loosely coupled.
